<!-- File này code chúa tể của sự lằng nhằng, Được viết vào ngày 25/08/2023 -->
<!-- Giờ bắt đầu tạo Job được giữ nguyên không sửa -->
<!-- JobID và currentDateOld đang để ẩn -->
<!-- Không sửa được phần Notes theo kiểu JS -->
<?php 
    ...
    $jobId = $_GET['jobId'];
    $jobs = getRecordTableById('tbl_job', 'id', $jobId);
    ...
    $A = array("Cẩn Thận", "Quan Trọng", "Ship gấp", "Lưu ý");
    $str = $job['note'];

?>
    <input type="hidden" id="jobId" value="<?php echo $jobId?>">
    <input type="hidden" id="currentDateOld" value="<?php echo $job['create_at']; ?>">
            ...
            <div class="col-md-12">
                <div class="mb-3">
                    ...
                    <textarea class="form-control" id="jobtitles" name="jobtitles" style="height: 20px;"
                        required><?php echo $job['job_titles'];?></textarea>
                </div>
            </div>
        </div>
        <div class="row">
            <div class="col-md-12">
                <div class="mb-3">
                    ...
                    <textarea class="form-control" id="jobdetails" name="jobdetails" style="height: 250px;"
                        required><?php echo $job['job_details'];?></textarea>
                </div>
            </div>
        </div>
        <div class="row">
            <div class="col-md-6">
                <div class="mb-3">
                    ...
                    <?php
                        if ($str !== null) {
                            $pieces = explode(", ", $str);
                            echo "<p>Hãy lựa chọn #tag bên dưới</p>";
                            
                            foreach ($pieces as $piece) {
                                if (in_array($piece, $A)) {
                                    // đã xoá đi Class tagButton, nên JS không được gọi để di chuyển tagButton
                                    echo '<button type="button" class="btn btn-light btn-info-custom ">' . $piece . '</button>';
                                }
                            }
                        } 
                    ?>
                    ...
                    <div class="listItem">
                        <!-- Mã PHP ở đây ẩn đi, vì không muốn cho người dùng Update ghi chú nữa. -->
                        <!-- Sau này muốn dùng thì chỉ cần thêm Class tagButton và code bên trên và hiện code này lên nhé -->
                        <!-- Notes: at 25/08/2023 -->
                    <!-- <php
                        if ($str !== null) {
                            $pieces = explode(", ", $str);
                            ...
                            foreach ($A as $tag) {
                                if (!in_array($tag, $pieces)) {
                                    echo '<button type="button" class="btn btn-light btn-info-custom tagButton">' . $tag . '</button>';
                                }
                            }
                        } 
                    ?> -->
            ...
            <div class="col-md-6">
                ...
                <div class="mb-3">
                    ...
                    <input type="date" class="form-control" id="deadline" name="deadline"
                        min="<?php echo substr($job['create_at'], 0, 10); ?>" max="" required
                        value="<?php echo $job['job_deadline']; ?>">
                </div>
            </div>
            <div class="mb-3 text-center">
                <button id="addJobButton" type="submit" class="btn btn-primary ">Update Job</button>
            </div>
        </div>
    </form>
</div>
...
document.getElementById('upload-form').addEventListener('submit', function(e) {
    ...
    var currentDateOld = document.getElementById('currentDateOld').value;
    ...
    formDataInput.append('currentDateOld', currentDateOld);
    formDataInput.append('jobId', jobId);

    var xhr_formDataInput = new XMLHttpRequest();

    xhr_formDataInput.open('POST', '/thinhcuong/job/job_update_process.php', true);
    ...
    xhr_formDataInput.onload = function() {
        if (xhr_formDataInput.status === 200) {
            ...
            // // Lại bắt đầu chết nhục với chỗ nào đây.
             ...
             // Giả sử bạn đã có jobId từ phản hồi trước đó
