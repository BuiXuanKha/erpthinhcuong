...
<div class="container custom-fixed-top">
    <form id="upload-form" method="POST" enctype="multipart/form-data">
        <div class="row text-center">
            <div class="col-md-12">
                <div class="mb-3">
                    <h1>THÊM SẢN PHẨM MỚI THỊNH CƯỜNG</h1>
                    <hr>
                </div>
            </div>
        </div>
            ...
            <div class="col-md-6">
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control" id="product_code" name="product_code"
                        value="Chưa chọn kiểu sản phẩm" required disabled>
                </div>
            </div>
            <div class="col-md-6">
                <div class="mb-3">
                    <label for="product_code" class="form-label">Select style product:</label>
                    <select id="product_style_select" class="form-select" onchange="handleProductStyleChange()">
                        <!-- <option value="">Chọn kiểu sản phẩm</option> -->
                        <?php
                            $vendorCodes = getRecordTableById('tbl_product_vendor_code','status','0');
                            if($vendorCodes){
                                // <option value="a">a</option>
                               foreach($vendorCodes as $vendorCode){
                                    echo "<option value=\"".$vendorCode['code']."\">".getAllTableById($vendorCode['id'],$vendorCodes,'code')." - ".$vendorCode['fullname']."</option>";
                               }
                            }
                        ?>
                    </select>
        ...
        <div class="row">
            <div class="col-md-6">
                <div class="mb-3">
                    ...
                    <input type="number" class="form-control" id="netweight" name="netweight" required min="0">
                </div>
            </div>
            <div class="col-md-6">
                <div class="mb-3">
...
<script>
// START -  XỬ LÝ TẠO MÃ SẢN PHẨM MỚI
document.addEventListener("DOMContentLoaded", function() {
    handleProductStyleChange(); // Gọi hàm khi trang được tải
});

function TaoMaSanPhamMoi(selectedValue) {
    return new Promise(function(resolve, reject) {
        var formData = new FormData();
        formData.append('key', selectedValue);
        var xhrformData = new XMLHttpRequest();
        xhrformData.open('POST', '/erpthinhcuong/product/generateNewProductCode.php', true);
        xhrformData.send(formData);
        xhrformData.onload = function() {
            if (xhrformData.status === 200) {
                var response = JSON.parse(xhrformData.responseText);
                var newProductCode = response.result; // Lấy giá trị mã sản phẩm từ response
                resolve(newProductCode);
            } else {
                reject("Error fetching product code");
            }
        };
    });
}

function handleProductStyleChange() {
    var selectElement = document.getElementById("product_style_select");
    var inputElement = document.getElementById("product_code");

    var selectedValue = selectElement.value;
    if (selectedValue == "") {
        console.log("Giá Trị của Select:" + selectedValue);
        inputElement.value = "Chưa chọn kiểu sản phẩm đâu bạn";
    } else {

        TaoMaSanPhamMoi(selectedValue).then(function(newProductCode) {

            inputElement.value = newProductCode;
        }).catch(function(error) {
            console.error(error);
        });
    }
}
// THE END -  XỬ LÝ TẠO MÃ SẢN PHẨM MỚI
...
document.getElementById('upload-form').addEventListener('submit', function(e) {
    ...
    xhr_formDataInput.open('POST', '/erpthinhcuong/product/product_addnew_process.php', true);
        ...
        if (xhr_formDataInput.status === 200) {
            ...
            if (selectedFilesImages.length <= 0) {
                ...
                if (selectedFilesImages.length > 0) {
                    ...
                    xhr_image.open('POST', '/erpthinhcuong/product/product_addnew_fileImages.php', true);
